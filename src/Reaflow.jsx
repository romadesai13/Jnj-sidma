...
function generatePrevGroupId(dataNode) {
    if (dataNode.Scenario_Prev) {
        return `${dataNode.Scenario_Prev}`;
    }
    else return '--';
}
...
function constructInitialNodes() {
    ...
    if (jsonData) {
        jsonData.map((n) => {
            ...
            const prevGroupId = generatePrevGroupId(n);
            const nodeId = generateNodeId(n);
            ...
            if (prevGroupId != "--" && prevGroupId !== groupId) {
                const edgeId = `${groupId}-${prevGroupId}`;
                const edge = {
                    id: edgeId,
                    from: prevGroupId,
                    to: groupId,
...
const Reaflow = () => {
    ...
    return (
        <div style={{
            ...
            <Canvas direction="RIGHT" nodes={initialNodes} edges={initialEdges} onLayoutChange={layout => console.log('Layout', layout)} fit />
        </div>
    );
};
